  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (data?.whatsapp_number && !error) {
          setPhone(data.whatsapp_number);
        }
      } catch (err) {
        console.warn('WhatsApp button settings fetch error:', err);
      }
    };
    fetchSettings();
  }, []);
  ...
  if (variant === 'button') {
    return (
      <a
        ...
        className="flex items-center justify-center gap-2 bg-brand-green hover:bg-brand-green/90 text-white px-4 md:px-6 py-2 md:py-2.5 rounded-lg font-bold transition-all w-full whitespace-nowrap text-xs md:text-sm overflow-hidden shadow-md"
      >
        <WhatsAppIcon className="w-3.5 h-3.5 md:w-5 md:h-5 shrink-0" />
